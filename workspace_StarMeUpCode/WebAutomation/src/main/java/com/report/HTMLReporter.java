...
/*import org.testng.internal.p.getMePropertyFile();*/
import org.testng.internal.Utils;
import org.testng.reporters.util.StackTraceTools;
import org.testng.xml.XmlSuite;

import Utility.ReadProperty;
//import utils.p.getMePropertyFile();
...
public class HTMLReporter extends TestListenerAdapter implements IReporter
{
    ...
    public void generateReport(List<XmlSuite> arg0, List<ISuite> suites, String outdir)
    {
        try
        {
            outputDir = System.getProperty("user.dir"); //p.getMePropertyFile().getProperty("app.test.reports.directory");
        } catch (Exception ex)
        {
            ex.printStackTrace();
        }
        ...
        try
        {
            modulesCommaSeparated = p.getMePropertyFile().getProperty("app.application.test.modules").replaceAll("\\s+", "");
            testGroupsCommaSeparated = p.getMePropertyFile().getProperty("app.application.test.groups").replaceAll("\\s+", "");
        } catch (Exception ex)
        {
            ex.printStackTrace();
        }
                ...
                } else
                {
                    for (String key : dashboardReportMap.keySet())
                    {
                        ...
                        if (key.equalsIgnoreCase(includedModule))
                        {
                            ...
                            int index = org.apache.commons.lang3.StringUtils.countMatches(value, "#") + 1;//Change
                            ...
                            if (grp.equalsIgnoreCase(modulename))
                            {
                                ...
                                while (statusIterator.hasNext())
                                {
                                    ...
                                    while (it2.hasNext())
                                    {
                                        ...
                                        try
                                        {
                                            if ((p.getMePropertyFile().getProperty("app.enable.logs.in.report")
            ...
            Throwable exception, ITestNGMethod method, String nodeIp, ITestResult result) throws IOException
    {
        ...
        try
        {
            if (p.getMePropertyFile().getProperty("app.enable.logs.in.report").equalsIgnoreCase("false"))
            {
    ...
    private void generateAdditionalInfoReport(List<ISuite> suites) throws Exception
    {
        ...
        if (url == null)
        {
            url = p.getMePropertyFile().getProperty("app.url");
        }
